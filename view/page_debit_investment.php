...
{
?>
<input type="radio" name="fromAccount" <?= $account->getAccountId() == $_SESSION['account_id'] ? 'checked' : '' ?> value="<?= $account->getAccountId() ?>"><?= $account->getName() ?><?= strlen($account->getDescription()) > 0 ? ' ('.$account->getDescription().')' : ''  ?><br />
<?php
}
?>
<br/>
